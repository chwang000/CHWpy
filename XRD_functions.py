...
def GaussianProfile(xo,fwhm,x_cut_off,x_interval):
    out = []
    j = 0
...
def LorentzProfile(xo,fwhm,x_cut_off,x_interval):
    out = []
    j = 0
    l1 = 2 / math.pi
    l2 = 4
...
#Pseudo-Voigt
def PVProfile(xo,fwhm,NA,NB,x_cut_off,x_interval):
    out = []
    j = 0
    g1 = 2 * math.sqrt(math.log(2)/math.pi)
    g2 = 4 * math.log(2)
...
def TCHZProfile(xo,U,V,W,X,Y,Z,x_cut_off,x_interval):
    out = []
    j = 0
    A = 2.69269
    B = 2.42843
...
def GProfileRange(xo,fwhm,x_min,x_max,num_decimal):
    out = []
    x_interval = 1 / (10 ** num_decimal)
    j = 0
    g1 = 2 * math.sqrt(math.log(2)/math.pi)
    g2 = 4 * math.log(2)
    num_interval = math.ceil((x_max - x_min) / x_interval)
    for i in range(0, num_interval+1):
        x = x_min + i * x_interval
        y = g1 / (fwhm * math.sqrt(math.pi)) * math.exp(-g2 * (x - xo)**2 / (fwhm**2))
        if j == 0:
            out = [x,y]
        else:
    ...
    out[:,1] = out[:,1] /y_max
    return out
def LProfileRange(xo,fwhm,x_min,x_max,num_decimal):
    out = []
    j = 0
    l1 = 2 / math.pi
    l2 = 4
...
#Pseudo-Voigt
def PVProfileRange(xo,fwhm,NA,NB,x_min,x_max,num_decimal):
    out = []
    j = 0
    g1 = 2 * math.sqrt(math.log(2)/math.pi)
    g2 = 4 * math.log(2)
...
#Pseudo-Voigt in refinement format
def PV_peakRange(xo,ha,hb,hc,lora,lorb,lorc,x_min,x_max,num_decimal):
    out = []
    j = 0
    g1 = 2 * math.sqrt(math.log(2)/math.pi)
    g2 = 4 * math.log(2)
...
#TCHZ profile
#ref: TCHZ_Peak_Type(pku, 0.00039,pkv, -0.00221,pkw, -0.00146,!pkz, 0.0000,pky, 0.00957,!pkx, 0.0000)
def TCHZProfileRange(xo,U,V,W,X,Y,Z,x_min,x_max,num_decimal):
    out = []
    j = 0
    A = 2.69269
    B = 2.42843
...
if __name__ == "__main__":
    ...
    X = TCHZProfileRange(15.0,0.00039,-0.00221,-0.00146,0,0.00957,0,0.5,80,2.5)
    Y = TCHZProfileRange(25.0,0.00039,-0.00221,-0.00146,0,0.00957,0,0.5,80,2.5)
    Z = TCHZProfileRange(45.0,0.00039,-0.00221,-0.00146,0,0.00957,0,0.5,80,2.5)
    P = PV_peak(5,0.02,0.02,0.02,0.02,0.02,0.02,0.5,0.01)
    T = TCHZProfile(5.0,0.00039,-0.00221,-0.00146,0,0.00957,0,0.5,0.001)
    print(Z)
    print(X.shape)
